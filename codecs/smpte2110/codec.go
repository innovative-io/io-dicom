...
var errUnsupportedTransferSyntax = errors.New("unsupported SMPTE ST 2110 transfer syntax")
var errBackendUnavailable = errors.New("smpte st 2110 decode requires the st2110 native backend (build with -tags st2110)")
...
// Backend defines a SMPTE ST 2110 implementation that can be plugged at runtime.
...
// SetBackend overrides the active SMPTE ST 2110 backend. Passing nil resets to default passthrough behavior.
...
// BackendName returns the current SMPTE ST 2110 backend name.
...
func isSupportedSMPTEUID(uid string) bool {
	switch uid {
	case "1.2.840.10008.1.2.7.1", "1.2.840.10008.1.2.7.2", "1.2.840.10008.1.2.7.3":
		return true
	default:
		return false
	}
...
func SMPTE2110decode(streamData []byte, streamSize uint32, outputData []byte, transferSyntaxUID string) error {
	return SMPTE2110decodeContext(context.Background(), streamData, streamSize, outputData, transferSyntaxUID)
}

func SMPTE2110decodeContext(ctx context.Context, streamData []byte, streamSize uint32, outputData []byte, transferSyntaxUID string) error {
	if !isSupportedSMPTEUID(transferSyntaxUID) {
...
func SMPTE2110encode(rawData []byte, width uint16, height uint16, samples uint16, bitsa uint16, outData *[]byte, outSize *int, transferSyntaxUID string) error {
	return SMPTE2110encodeContext(context.Background(), rawData, width, height, samples, bitsa, outData, outSize, transferSyntaxUID)
}

func SMPTE2110encodeContext(ctx context.Context, rawData []byte, width uint16, height uint16, samples uint16, bitsa uint16, outData *[]byte, outSize *int, transferSyntaxUID string) error {
	if !isSupportedSMPTEUID(transferSyntaxUID) {
		return errUnsupportedTransferSyntax
	}
